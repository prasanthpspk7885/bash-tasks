...
sudo systemctl enable httpd" | base64 -w0)
    ...
    instance_id=$(aws ec2 run-instances --image-id ami-0efbb9b523fbc6c53 --instance-type t2.micro --key-name pspk --subnet-id "$public_subnet_id" --security-group-ids "$sg_id" --associate-public-ip-address --user-data "$user_data" --query 'Instances[0].InstanceId' --output text)
